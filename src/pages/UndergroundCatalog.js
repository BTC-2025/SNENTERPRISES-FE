...
import { undergroundCatalog, formatPrice, getAllUndergroundProducts, getUndergroundSizes, getUndergroundFittingTypes, getUndergroundStiffnessClasses } from '../data/undergroundCatalog';
import { companyInfo } from '../data/content';
import './UndergroundCatalog.css';

const UndergroundCatalog = () => {
    ...
    const allItems = useMemo(() => getAllUndergroundProducts(), []);
    const sizes = useMemo(() => getUndergroundSizes(), []);
    const fittingTypes = useMemo(() => getUndergroundFittingTypes(), []);
    const stiffnessClasses = useMemo(() => getUndergroundStiffnessClasses(), []);
    ...
    return (
        <div className="underground-catalog-page">
            <section className="catalog-hero">
                <div className="catalog-hero-parallax" ref={parallaxRef}></div>
                <div className="container">
                    <div className="breadcrumb">
                        <Link to="/">Home</Link> / <Link to="/products">Products</Link> / <Link to="/products/underground">Underground</Link> / Catalog
                    </div>
                    <div className="hero-content-flex">
                        <div>
                            <h1>{undergroundCatalog.title}</h1>
                            <p className="hsn-code">HSN Code: {undergroundCatalog.hsn_code} • {undergroundCatalog.certification}</p>
                            <p className="catalog-subtitle">{undergroundCatalog.description}</p>
                            <p className="catalog-subtitle">Complete catalog with {allItems.length} underground drainage pipes</p>
                        </div>
            ...
            <section className="section catalog-content">
                <div className="container">
                    {filteredItems.length === 0 ? (
                        ...
                        <UndergroundTable items={filteredItems} />
                    )}
                </div>
            </section>
            ...
            <section className="section-sm catalog-cta">
                <div className="container text-center">
                    <h2>Need Underground Drainage Pipes?</h2>
                    <p>Contact us for special rates on bulk orders</p>
                    <div className="cta-buttons">
                        <Link to="/quote" className="btn btn-primary btn-lg">Request Quote</Link>
                        <a href={`https://wa.me/${companyInfo.contact.whatsapp}?text=Hi, I need pricing for Underground Drainage Pipes`} target="_blank" rel="noopener noreferrer" className="btn btn-lg whatsapp-btn">WhatsApp Inquiry</a>
                    </div>
                </div>
            </section>
        </div>
...
export default UndergroundCatalog;
