...
import { drainproCatalog, formatPrice, getAllDrainProProducts, getDrainProSizes, getDrainProSocketTypes } from '../data/drainproCatalog';
import { companyInfo } from '../data/content';
import './DrainProCatalog.css';

const DrainProCatalog = () => {
    const parallaxRef = useParallax({ speed: 0.4 });
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedSize, setSelectedSize] = useState('all');
    const [selectedSocket, setSelectedSocket] = useState('all');

    const allItems = useMemo(() => getAllDrainProProducts(), []);
    const sizes = useMemo(() => getDrainProSizes(), []);
    const socketTypes = useMemo(() => getDrainProSocketTypes(), []);
    ...
    const filteredItems = useMemo(() => {
        return allItems.filter(item => {
            ...
            const matchesSocket = selectedSocket === 'all' || item.length_socket === selectedSocket;

            return matchesSearch && matchesSize && matchesSocket;
        });
    }, [allItems, searchTerm, selectedSize, selectedSocket]);
    ...
    return (
        <div className="drainpro-catalog-page">
            <section className="catalog-hero">
                <div className="catalog-hero-parallax" ref={parallaxRef}></div>
                <div className="container">
                    <div className="breadcrumb">
                        <Link to="/">Home</Link> / <Link to="/products">Products</Link> / <Link to="/products/drainpro">DrainPro</Link> / Catalog
                    </div>
                    <div className="hero-content-flex">
                        <div>
                            <h1>{drainproCatalog.title}</h1>
                            <p className="hsn-code">HSN Code: {drainproCatalog.hsn_code}</p>
                            <p className="catalog-subtitle">{drainproCatalog.description}</p>
                            <p className="catalog-subtitle">Pipes catalog with {allItems.length} products</p>
                        </div>
                ...
                <div className="container">
                    <div className="filters-bar">
                        <div className="search-box">
                            <Search size={20} />
                            <input type="text" placeholder="Search by size, socket type, product code..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="search-input" />
                        </div>
                        ...
                        <div className="filter-group">
                            ...
                            <select value={selectedSocket} onChange={(e) => setSelectedSocket(e.target.value)} className="filter-select">
                                <option value="all">All Socket Types</option>
                                {socketTypes.map(type => (<option key={type} value={type}>{type}</option>))}
                            </select>

                            {(searchTerm || selectedSize !== 'all' || selectedSocket !== 'all') && (
            ...
            <section className="section catalog-content">
                <div className="container">
                    {filteredItems.length === 0 ? (
                        ...
                        <DrainProTable items={filteredItems} />
                    )}
                </div>
            </section>
            ...
            <section className="section-sm catalog-cta">
                <div className="container text-center">
                    <h2>Need DrainPro SWR Pipes?</h2>
                    <p>Contact us for special rates on bulk orders</p>
                    <div className="cta-buttons">
                        <Link to="/quote" className="btn btn-primary btn-lg">Request Quote</Link>
                        <a href={`https://wa.me/${companyInfo.contact.whatsapp}?text=Hi, I need pricing for DrainPro SWR Pipes`} target="_blank" rel="noopener noreferrer" className="btn btn-lg whatsapp-btn">WhatsApp Inquiry</a>
                    </div>
                </div>
            </section>
        </div>
...
export default DrainProCatalog;
